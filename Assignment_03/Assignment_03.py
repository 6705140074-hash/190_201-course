...
class Order:
    ...
    def tax(self):
        ...
        for item in self.items:
            if item.product.category != FOOD_CATEGORY:
                tax = tax + item.product.price * item.quantity * TAX_RATE
        return tax
    ...
    def points(self):
        ...
        return base_points * self.customer.point_multiplier()
    ...
    def receipt(self):
        ...
        for item in self.items:
            ...
            print(item.product.name + " x" + str(item.quantity) + " = " + str(line))
...
def refactored_main():
    ...
    for name, tier, old_items in ORDERS:
        customer = customer_types[tier](name)

        items = []
        for product_number, quantity in old_items:
            item = OrderItem(products[product_number], quantity)
            items.append(item)
